#!/usr/bin/env python3
"""
ATRX CNN Training Script
=======================

Trains 1D CNN models for financial time series prediction using walk-forward validation.
Implements convolutional sequence learning optimized for temporal pattern recognition.
...
class CNNTrainer(BaseTrainer):
    """1D CNN model trainer for financial time series."""
    ...
    def __init__(self, *args, sequence_length: int = 64, **kwargs):
        ...
        """
        super().__init__(*args, model_type='cnn', **kwargs)
    ...
    def build_model(self, input_shape: Tuple[int, int]) -> keras.Model:
        ...
        model = Sequential(name=f'CNN1D_{self.sequence_length}')
        
        # Input layer
        model.add(layers.Input(shape=input_shape, name='input'))
        
        # Convolutional layers
        conv_filters = config.get('conv_filters', [64, 128, 64])
        ...
        max_layers = max(len(conv_filters), len(kernel_sizes), len(pool_sizes))
        conv_filters = (conv_filters * max_layers)[:max_layers]
        kernel_sizes = (kernel_sizes * max_layers)[:max_layers]
        pool_sizes = (pool_sizes * max_layers)[:max_layers]
        ...
        # Global pooling to handle variable sequence lengths after convolution
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Train 1D CNN model for ATRX trading system',
